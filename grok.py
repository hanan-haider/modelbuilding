...
class CLIP_Inplanted(nn.Module):  # NEW: Add dropout
    def __init__(self, clip_model, features):
        super(CLIP_Inplanted, self).__init__()
        self.clip_model = clip_model
        self.features = features
        self.seg_adapters = nn.ModuleList([nn.Linear(512, 512) for _ in features])  # Assuming embed_dim=512 for BiomedCLIP
        self.det_adapters = nn.ModuleList([nn.Linear(512, 512) for _ in features])
        self.dropout = nn.Dropout(0.1)  # NEW: Dropout for regularization

    def forward(self, x):
        # Extract features from BiomedCLIP (use hooks for layers)
        features = []
        def hook(module, input, output):
            features.append(output)
        handles = [self.clip_model.visual.transformer.resblocks[i-1].register_forward_hook(hook) for i in self.features]
        _ = self.clip_model.encode_image(x)
        for h in handles:
            h.remove()

        seg_out = [self.dropout(self.seg_adapters[i](f)) for i, f in enumerate(features)]
        det_out = [self.dropout(self.det_adapters[i](f)) for i, f in enumerate(features)]
        return None, seg_out, det_out  # Adjust if needed
